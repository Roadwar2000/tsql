use comm_reviewfileprocess
go
select 'declare @dt varchar(50); set @dt=convert(varchar(50), getdate(),100); raiserror(''comm_reviewfileprocess %s'',0,0,@dt);'
union all
select 'go'
union all
select 'use comm_reviewfileprocess'
union all
select 'go'
union all
...
FROM sys.dm_db_index_physical_stats(DB_ID('comm_reviewfileprocess'), NULL, NULL, NULL, 'DETAILED') phystat inner JOIN sys.indexes i 
